...
import type { IAction }   from "@core-api/action-types";
import type {
	AudioAssetID,
	FadeOptions,
	IAudioPlayer,
...
export class MusicManager implements IMusicManager,
	...
	private readonly _onMusicEnd?:(musicAlias:AudioAssetID, completed:boolean) => void;
	...
	constructor(
		...
		onMusicEnd:(musicAlias:AudioAssetID, completed:boolean) => void,
		onMusicStop:() => void
	) {
		this.name = name;
		this._voice = voice;
	...
	update(time:GameTime):void {
		...
		if(this._fadeOutAction) {
			this._fadeOutAction.update?.(time.deltaTimeMs);
		...
		if(this._fadeInAction) {
			this._fadeInAction.update?.(time.deltaTimeMs);
